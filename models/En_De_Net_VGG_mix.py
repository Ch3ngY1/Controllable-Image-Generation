...
class UNet(nn.Module):
    def __init__(self, args):
        ...
        self.ushape = nn.Conv2d(512, round(512*ratio), (1, 1), 1)
        self.ufeature = nn.Conv2d(512, round(512*(1-ratio)), (1, 1), 1)
    ...
    def forward(self, x, y, cls=False):
        ...
        # loss： x和x正交， shapex和featurex可以组成out4
...
class Model(nn.Module):
    ...
    def forward(self, x, y, target_x, target_y, GAN=False):
        ...
        if self.training and GAN:
            ...
            loss_gen = lossGen1 * self.weight_pred[0] + lossGen2 / ((x_gen_cls ** 2).mean() + (y_cls ** 2).mean()) * \
                       self.weight_pred[1] + loss_orth * 100.
