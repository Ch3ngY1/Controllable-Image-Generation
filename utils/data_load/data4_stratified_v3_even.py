...
class MyDataset(data_utils.Dataset):
    ...
    def __init__(self, img_root, data_root, dataset, transform=None, fold=3):
        ...
        root = '/data2/chengyi/dataset/ord_reg/aesthetics/stratified/new_8020/'
...
if __name__ == '__main__':
    ...
    for fold in range(5):
        ...
        for row in reader:
            # print(row)
            label = int(row[-1])
            sub[label].append(row[3])
            # items.append(row[3]+row[-1])
