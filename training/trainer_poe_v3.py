...
class DefaultTrainer(object):
    ...
    def __init__(self, args):
        ...
        if self.main_loss_type == 'cls':
            ...
            numclass = args.num_classes * 2
        elif self.main_loss_type == 'reg':
            numclass = 1
        else:
            raise ValueError
        self.poe = args.no_poe
        self.gan = args.no_GAN
        self.model = getattr(models, args.model_name.lower())(main_loss_type=self.main_loss_type,
                                                              numclass=numclass, poe=args.no_poe,
                                                              use_GAN=args.no_GAN)
        ...
        self.metric = get_metric(self.main_loss_type)
    ...
    def train_iter(self, step, dataloader):
        ...
        logit, emb, log_var, loss = self.model(img, img2, label, label2, mh, mh2)
        ...
        if step % self.args.display_freq == 0:
            mae, acc = self.metric(logit, label, self.poe)
    ...
    def validation(self, step, val_iter, val_epoch_size):
        ...
        with torch.no_grad():
            for i in range(val_epoch_size):
                ...
                logit, emb, log_var, loss_tmp = self.model(img, img2, target, target2, mh, mh2)
                ...
                if i == 0:
                    total_logit = logit
                    total_embedding = emb
                    # total_log_var = log_var
                    total_target = target
                else:
                    total_logit = torch.cat([total_logit, logit], dim=1)
                    # total_embedding = torch.cat([total_embedding, emb], dim=0)
                    # total_log_var = torch.cat([total_log_var, log_var], dim=0)
                    total_target = torch.cat([total_target, target], 0)

        mae, acc = self.metric(total_logit, total_target, self.poe)
        ...
        if self.args.exp_name == 'test':
            ...
            log_path = os.path.join(self.args.save_log,
                                    datetime.now().strftime(
                                        '%b%d_%H-%M-%S') + '_' + self.args.optim + '_' + self.args.exp_name)
